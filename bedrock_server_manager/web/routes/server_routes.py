...
def restart_server_route(server_name):
    ...
    if response["status"] == "success":
        ...
        flash(f"Error restarting server '{server_name}': {response['message']}", "error")
    return redirect(url_for("server_routes.index"))
...
def install_server_route():
    ...
    if request.method == "POST":
        ...
        if validation_response["status"] == "error":
            ...
            return render_template("install.html", server_name=server_name, server_version=target_version) #pass back

        # Check if server exists using the handler
        server_exists_response = handlers.validate_server_name_handler(server_name, base_dir)
        if server_exists_response["status"] == "success":
            flash(f"Server '{server_name}' already exists.", "error")
            return render_template("install.html", server_version=target_version) #pass back
        ...
        if install_response["status"] == "error":
            ...
            return render_template("install.html", server_name=server_name, server_version=target_version)

        # Redirect to configure_properties_route, passing server_name
        return redirect(url_for("server_routes.configure_properties_route", server_name=server_name))
    ...
    if request.method == 'POST':
        ...
        for key in allowed_keys:
            ...
            if value is not None: # Key exists in form.
                ...
                validation_response = handlers.validate_property_value_handler(key, value)
                if validation_response["status"] == "error":
                    flash(validation_response["message"], "error")
                    # Re-render form WITH existing, and current properties
                    current_properties = handlers.read_server_properties_handler(server_name, base_dir)["properties"]
                    return render_template("configure_properties.html", server_name=server_name, properties=current_properties)
                properties_to_update[key] = value

        # Call the handler to modify properties
        modify_response = handlers.modify_server_properties_handler(server_name, properties_to_update, base_dir)
        if modify_response["status"] == "error":
            flash(f"Error updating server properties: {modify_response['message']}", "error")
            # Re-render form with current values
            current_properties = handlers.read_server_properties_handler(server_name, base_dir)["properties"]
            return render_template("configure_properties.html", server_name=server_name, properties=current_properties)
        # Write server config
        config_dir = settings.get("CONFIG_DIR")
        write_config_response = handlers.write_server_config_handler(server_name,"server_name", server_name, config_dir)
        ...
        write_config_response = handlers.write_server_config_handler(server_name, "target_version", level_name, config_dir)
        if write_config_response["status"] == "error":
            flash(write_config_response['message'], "error")
        write_config_response = handlers.write_server_config_handler(server_name, "status", "INSTALLED", config_dir)
        if write_config_response["status"] == "error":
            flash(write_config_response['message'], "error")
        ...
        return render_template("configure_properties.html", server_name=server_name, properties=properties_response["properties"])