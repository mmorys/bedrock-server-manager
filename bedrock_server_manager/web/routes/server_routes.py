...
def send_command_route(server_name):
    ...
    if not command:
        logger.warning(f"Received send_command request for {server_name} with empty command.")
...
def install_server_route():
    if request.method == "POST":
        ...
        if os.path.exists(server_dir):
            logger.info(f"Server {server_name} already exists.  Prompting for confirmation.")
        ...
        if result["status"] == "error":
            ...
            logger.info(f"Server {server_name} requires confirmation to proceed (should not occur here).")
            return render_template(
                "install.html",
                confirm_delete=True,
            ...
            logger.info(f"Server {server_name} installed successfully.  Redirecting to configure_properties.")
            return redirect(
                url_for(
...
def confirm_install_route():
    ...
    if confirm == "yes":
        ...
        if delete_result["status"] == "error":
            ...
            logger.error(f"Error deleting existing server data for {server_name}: {delete_result['message']}")
            return render_template(
                "install.html", server_name=server_name, server_version=server_version
            )
        ...
        if install_result["status"] == "error":
            ...
            logger.error(f"Error installing server {server_name} after deletion: {install_result['message']}")
            return render_template(
                "install.html", server_name=server_name, server_version=server_version
            )

        elif install_result["status"] == "success":
            logger.info(f"Server {server_name} reinstalled successfully.  Redirecting to configuration.")
            return redirect(
                url_for(
                    "server_routes.configure_properties_route",
            ...
            logger.error("An unexpected error occurred during installation after deletion.")
        ...
        logger.warning(f"Invalid confirmation value for server {server_name}: {confirm}")
...
def configure_properties_route(server_name):
    ...
    logger.info(f"Configuring properties for server: {server_name}, new_install: {new_install}")
    if request.method == "POST":
        ...
        for key in allowed_keys:
            ...
            if value is not None:
                ...
                if validation_response["status"] == "error":
                    ...
                    logger.error(f"Validation error for {key}={value}: {validation_response['message']}")
        ...
        if modify_response["status"] == "error":
            ...
            logger.error(f"Error updating server properties for {server_name}: {modify_response['message']}")
            current_properties = handlers.read_server_properties_handler(
                server_name, base_dir
            )["properties"]
            return render_template(
        ...
        if write_config_response["status"] == "error":
            ...
            logger.error(f"Error writing server_name to config: {write_config_response['message']}")
        ...
        if write_config_response["status"] == "error":
            flash(write_config_response["message"], "error")
            logger.error(f"Error writing target_version to config: {write_config_response['message']}")
        ...
        if write_config_response["status"] == "error":
            flash(write_config_response["message"], "error")
            logger.error(f"Error writing status to config: {write_config_response['message']}")
        ...
        if properties_response["status"] == "error":
            ...
            logger.error(f"Error loading properties for {server_name}: {properties_response['message']}")
            return redirect(url_for("server_routes.advanced_menu_route"))
...
def configure_allowlist_route(server_name):
    ...
    new_install = new_install_str.lower() == "true"
    logger.info(f"Configuring allowlist for server: {server_name}, new_install: {new_install}")
    ...
    if request.method == "POST":
        ...
        if result["status"] == "success":
            ...
            logger.error(f"Error updating allowlist for {server_name}: {result['message']}")
...
def configure_permissions_route(server_name):
    ...
    logger.info(f"Configuring permissions for server: {server_name}, new_install: {new_install}")
    ...
    if request.method == "POST":
        ...
        for xuid, permission in permissions_data.items():
            ...
            if result["status"] == "error":
                ...
                logger.error(f"Error setting permission for {player_name} ({xuid}): {result['message']}")
...
def configure_service_route(server_name):
    ...
    new_install_str = request.args.get("new_install", "False")
    new_install = new_install_str.lower() == "true"
    logger.info(f"Configuring service for server: {server_name}, new_install: {new_install}")
    if request.method == "POST":
        if platform.system() == "Linux":
            ...
            if response["status"] == "error":
                ...
                logger.error(f"Error creating systemd service for {server_name}: {response['message']}")
            ...
            if response["status"] == "error":
                flash(response["message"], "error")
                logger.error(f"Error setting Windows autoupdate for {server_name}: {response['message']}")
                # Re-render the form
                return render_template(
                    "configure_service.html",
                    server_name=server_name,
            ...
            logger.error(f"Unsupported OS for service configuration: {platform.system()}")
            return redirect(url_for("server_routes.advanced_menu_route"))
        ...
        if new_install:
            if request.form.get("start_server") == "on":  # Check if checkbox is checked
                ...
                if start_result["status"] == "error":
                    ...
                    logger.error(f"Error starting server {server_name} after install: {start_result['message']}")
        ...
        if platform.system() == "Linux":
            ...
            logger.debug(f"Rendering configure_service.html for Windows, autoupdate: {autoupdate}")
            return render_template(
                "configure_service.html",
                server_name=server_name,
                os=platform.system(),
...
def backup_action_route(server_name):
    ...
    if not backup_type:
        ...
        logger.warning(f"Invalid backup request for {server_name}: No backup type specified.")
        return redirect(url_for("server_routes.manage_server_route"))

    if backup_type == "config" and not file_to_backup:
        logger.warning(f"Invalid backup request for {server_name}: No file selected for config backup.")
        return redirect(
            url_for("server_routes.backup_config_select_route", server_name=server_name)
        )
    logger.info(f"Performing backup for server: {server_name}, type: {backup_type}, file: {file_to_backup}")
...
def restore_select_backup_route(server_name):
    ...
    if not restore_type:
        ...
        logger.warning(f"Restore request for {server_name} with no restore type selected.")
        return redirect(url_for("server_routes.index"))

    logger.info(f"Displaying backup selection for restore type '{restore_type}' for server: {server_name}")
    ...
    if restore_type == "all":
        ...
        if result["status"] == "error":
            ...
            logger.error(f"Error restoring all files for {server_name}: {result['message']}")
        else:
...
def restore_action_route(server_name):
    ...
    if not backup_file or not restore_type:
        ...
        logger.warning(f"Invalid restore request for {server_name}: Missing backup file or restore type.")
        return redirect(url_for("server_routes.manage_server_route"))

    logger.info(f"Restoring server: {server_name}, type: {restore_type}, file: {backup_file}")
...
def install_world_route(server_name):
    ...
    if request.method == "POST":
        ...
        if not selected_file:
            ...
            logger.warning(f"Install world request for {server_name} with no file selected.")
        ...
        if world_name_result["status"] == "success":  # world exists.
            ...
            if os.path.exists(world_path):
                ...
                logger.info(f"World already exists for {server_name}. Prompting for overwrite.")
                return render_template(
                ...
                logger.info(f"World does not exist for {server_name}. Proceeding with installation.")
                ...
                if result["status"] == "error":
                    ...
                    logger.error(f"Error importing world for {server_name}: {result['message']}")
                else:
            ...
            logger.error(f"Error getting world name for {server_name}: {world_name_result['message']}")
            return redirect(url_for("server_routes.install_content_menu_route"))
...
def confirm_world_overwrite_route(server_name):
    ...
    selected_file = request.form.get("selected_file")
    confirm = request.form.get("confirm")
    logger.info(f"Confirming world overwrite for server: {server_name}, file: {selected_file}, confirm: {confirm}")

    if not selected_file:
        flash("No world file selected.", "error")
        logger.warning(f"World overwrite confirmation request for {server_name} with no file selected.")
        return redirect(url_for("server_routes.install_content_menu_route"))
    ...
    if confirm == "yes":
        ...
        if result["status"] == "error":
            flash(f"Error importing world: {result['message']}", "error")
            logger.error(f"Error importing world (overwrite) for {server_name}: {result['message']}")
        else:
            flash("World imported successfully!", "success")
...
def install_addon_route(server_name):
    ...
    if request.method == "POST":
        ...
        if not selected_file:
            ...
            logger.warning(f"Addon install request for {server_name} with no file selected.")
            return redirect(url_for("server_routes.install_content_menu_route"))
        ...
        if result["status"] == "error":
            ...
            logger.error(f"Error installing addon for {server_name}: {result['message']}")
        else:
...
def schedule_tasks_route(server_name):
    ...
    if cron_jobs_response["status"] == "error":
        ...
        logger.error(f"Error getting cron jobs for {server_name}: {cron_jobs_response['message']}")
        ...
        if table_response["status"] == "error":
            ...
            logger.error(f"Error formatting cron jobs for {server_name}: {table_response['message']}")
            table_data = []
        else:
...
def add_cron_job_route(server_name):
    ...
    if not cron_string:
        logger.warning(f"Add cron job request for {server_name} with empty cron string.")
...
def modify_cron_job_route(server_name):
    ...
    logger.info(f"Modifying cron job for {server_name}. Old: {old_cron_string}, New: {new_cron_string}")
    if not old_cron_string or not new_cron_string:
        logger.warning(f"Modify cron job request for {server_name} with missing old or new cron string.")
        return (
            jsonify(
                {
                    "status": "error",
...
def delete_cron_job_route(server_name):
    ...
    if not cron_string:
        logger.warning(f"Delete cron job request for {server_name} with empty cron string.")
        return jsonify({"status": "error", "message": "Cron string is required."}), 400
...
def schedule_tasks_windows_route(server_name):
    ...
    if platform.system() != "Windows":
        ...
        logger.warning(f"Attempted to access Windows task scheduler from non-Windows system.")
        return redirect(url_for("server_routes.index"))
    ...
    if task_names_response["status"] == "error":
        ...
        logger.error(f"Error getting task names for {server_name}: {task_names_response['message']}")
        ...
        if task_info_response["status"] == "error":
            ...
            logger.error(f"Error getting task info for {server_name}: {task_info_response['message']}")
            tasks = []
        else:
...
def add_windows_task_route(server_name):
    ...
    if request.method == "POST":
        ...
        if result["status"] == "success":
            ...
            logger.error(f"Error adding task '{task_name}' for {server_name}: {result['message']}")
            return render_template("add_windows_task.html", server_name=server_name)
...
def delete_windows_task_route(server_name):
    ...
    if not task_name or not task_file_path:
        ...
        logger.warning(f"Invalid task deletion request for {server_name}: Missing task name or file path.")
        return redirect(url_for("server_routes.index"))
    ...
    if result["status"] == "error":
        ...
        logger.error(f"Error deleting task {task_name} for server {server_name}: {result['message']}")
    else:
