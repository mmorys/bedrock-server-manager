...
class PluginManager:
    ...
    def _synchronize_config_with_disk(self):
        ...
        for plugin_name, path in all_potential_plugin_files.items():
            ...
            if not plugin_class:
                logger.warning(
                    f"Could not find a valid PluginBase subclass in '{path}'. Skipping."
                )
                ...
                if plugin_name in self.plugin_config:
                    ...
                    config_changed = True
                    logger.info(
                        f"Removed invalid plugin entry '{plugin_name}' from config because its class could not be loaded."
                    )
                continue
            ...
            if (
                not version or not str(version).strip()
            ):  # Check if version is None, empty, or whitespace
                logger.warning(
                ...
                if plugin_name in self.plugin_config:
                    ...
                    config_changed = True
                    logger.info(
                        f"Removed plugin entry '{plugin_name}' from config due to missing/invalid version."
                    )
                continue  # Skip this plugin entirely

            valid_plugins_found_on_disk.add(
                plugin_name
            )  # Mark as valid for later cleanup
            ...
            if not isinstance(current_config_entry, dict):
                ...
                if current_config_entry is None:
                    logger.info(
                        f"Discovered valid plugin '{plugin_name}' v{version}. Added to config."
                    )
                else:
                    logger.info(
            ...
            # Case 2: Existing dictionary entry, check for missing keys or changed metadata
                ...
                if needs_update:
                    ...
                    config_changed = True
                    logger.info(
                        f"Updated metadata/config for plugin '{plugin_name}' to v{version}."
                    )
    ...
    def load_plugins(self):
        ...
        if self.plugins:  # Only log if there were plugins to clear
            logger.debug(
                f"Clearing {len(self.plugins)} previously loaded plugin instances before new load."
            )
            self.plugins.clear()
        ...
        for plugin_name, config_data in self.plugin_config.items():
            ...
            ):  # N/A might be a default from an older sync
                logger.warning(
                    f"Plugin '{plugin_name}' has missing or invalid version ('{plugin_version}') in config despite sync. Skipping load."
                )
                continue
            ...
            if not path:  # Should not happen if sync worked, but good to check
                logger.warning(
                    f"Enabled plugin '{plugin_name}' v{plugin_version} path not found. Skipping."
                )
                continue
            ...
            if plugin_class:
                ...
                logger.error(
                    f"Could not get class for plugin '{plugin_name}' at path '{path}' during load. Skipping."
                )
    ...
    def register_plugin_event_listener(
        ...
        if not callable(callback):
            logger.error(
                f"Plugin '{listening_plugin_name}' attempted to register non-callable for '{event_name}'."
            )
            return
        ...
        logger.info(
            f"Plugin '{listening_plugin_name}' registered listener for custom event '{event_name}'."
        )
    ...
    def trigger_custom_plugin_event(
        ...
        if event_name in _custom_event_context.stack:
            logger.debug(
                f"Skipping recursive custom event '{event_name}' by '{triggering_plugin_name}'."
            )
            return

        _custom_event_context.stack.append(event_name)
        logger.info(
            f"Plugin '{triggering_plugin_name}' triggering custom event '{event_name}'."
        )
        try:
            listeners = self.custom_event_listeners.get(event_name, [])
            logger.debug(
                f"Found {len(listeners)} listeners for custom event '{event_name}'."
            )
    ...
    def trigger_event(self, event: str, *args, **kwargs):
        ...
        try:
            logger.debug(
                f"Dispatching event '{event}'. Current stack: {_event_context.stack} (Plugins: {len(self.plugins)})"
            )
