...
def _cleanup_log_files(log_dir_path: Path) -> int:
    ...
    if not log_dir_path.is_dir():
        click.secho(f"Warning: Log directory '{log_dir_path}' does not exist.", fg="yellow")
    ...
    try:
        ...
        logger.error(f"Error during log cleanup in '{log_dir_path}': {e}", exc_info=True)
...
@click.option("-c", "--cache", is_flag=True, help="Clean up Python cache files (__pycache__).")
@click.option("-l", "--logs", is_flag=True, help="Clean up application log files (*.log).")
@click.option(
    "--log-dir",
...
def cleanup(cache: bool, logs: bool, log_dir_override: Path):
    ...
    if not cache and not logs:
        click.secho("No cleanup options specified. Use --cache, --logs, or both.", fg="yellow")
