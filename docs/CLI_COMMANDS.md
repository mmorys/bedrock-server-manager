...
| `prune`         | Deletes old backups for a server, keeping the newest N.                                                     | `-s, --server SERVER_NAME` (required): Name of the server whose backups to prune.<br>`-k, --keep KEEP_OVERRIDE`: Number of backups to keep (overrides settings).                                                                               | All      |
