...
def test_backup_server_api_route_success(mock_backup, client):
...
@patch("bedrock_server_manager.api.backup_restore.backup_all")
def test_backup_server_api_route_failure(mock_backup, client):
    """Test the backup_server_api_route with a failed backup."""
    mock_backup.return_value = {"status": "error", "message": "Backup failed"}
    response = client.post(
        "/api/server/test-server/backup/action", json={"backup_type": "all"}
    )
    assert response.status_code == 202
...
def test_restore_backup_api_route_success(mock_restore, client):
...
@patch("bedrock_server_manager.api.backup_restore.restore_all")
def test_restore_backup_api_route_failure(mock_restore, client):
    """Test the restore_backup_api_route with a failed restore."""
    mock_restore.return_value = {"status": "error", "message": "Restore failed"}
    response = client.post(
        "/api/server/test-server/restore/action", json={"restore_type": "all"}
    )
    assert response.status_code == 202
...
@patch("bedrock_server_manager.api.backup_restore.backup_all")
def test_backup_in_progress(mock_backup, client):
    """Test that a 423 is returned when a backup is in progress."""
    from bedrock_server_manager.error import BSMError

    mock_backup.side_effect = BSMError("Backup/restore operation already in progress.")
    response = client.post(
        "/api/server/test-server/backup/action", json={"backup_type": "all"}
    )
    assert response.status_code == 202
