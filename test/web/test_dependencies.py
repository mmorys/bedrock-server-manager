...
async def test_validate_server_exists(app_context, real_bedrock_server):
    ...
    result = await validate_server_exists(app_context, real_bedrock_server.server_name)
...
async def test_validate_server_not_found(app_context):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await validate_server_exists(app_context, "non-existent-server")
...
async def test_validate_server_invalid_name(app_context):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await validate_server_exists(app_context, "invalid name")
