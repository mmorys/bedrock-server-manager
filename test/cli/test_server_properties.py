...
def test_properties_get_success(runner):
    ...
    with patch("bedrock_server_manager.cli.server_properties.config_api.get_server_properties_api") as mock_get_props:
        mock_get_props.return_value = {"status": "success", "properties": {"gamemode": "survival"}}
...
def test_properties_get_not_found(runner):
    ...
    with patch("bedrock_server_manager.cli.server_properties.config_api.get_server_properties_api") as mock_get_props:
        ...
        result = runner.invoke(properties, ["get", "-s", "test-server", "-p", "not-found"])
...
def test_properties_set_success(runner):
    ...
    with patch("bedrock_server_manager.cli.server_properties.config_api.modify_server_properties") as mock_set_props, \
         patch("bedrock_server_manager.cli.server_properties._handle_api_response") as mock_handle_response:
        mock_set_props.return_value = {"status": "success"}

        result = runner.invoke(properties, ["set", "-s", "test-server", "-p", "gamemode=creative"])

        assert result.exit_code == 0
        mock_set_props.assert_called_once_with("test-server", {"gamemode": "creative"}, restart_after_modify=True)
...
def test_interactive_properties_workflow(runner):
    ...
    with patch("bedrock_server_manager.cli.server_properties.config_api.get_server_properties_api") as mock_get_props, \
         ...
         patch("bedrock_server_manager.cli.server_properties.config_api.modify_server_properties") as mock_modify:
        mock_get_props.return_value = {"status": "success", "properties": {"gamemode": "survival"}}
